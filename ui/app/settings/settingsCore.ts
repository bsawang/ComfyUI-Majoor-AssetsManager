...
export const saveMajoorSettings = (settings: Record<string, any>): void => {
    try {
...
export const applySettingsToConfig = (settings: Record<string, any>): void => {
    ...
    const _safeColor = (v: any, fallback: any) => {
        let c = String(v || "").trim();
    ...
    try {
        const roots = Array.from(document.querySelectorAll<HTMLElement>(".mjr-assets-manager"));
...
export async function syncBackendVectorSearchSettings(): Promise<void> {
    try {
...
export async function syncBackendExecutionGroupingSettings(): Promise<void> {
    try {
